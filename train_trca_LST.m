function model = train_trca_LST(template, supplements, fs, num_fbs)
...
for targ_i = 1:1:num_targs
    ...
    for fb_i = 1:1:num_fbs
        ...
        transferred_eeg_tmp = zeros(num_chans, num_smpls, size(supplement_tmp, 3));
        
        for trialIdx = 1 : size(supplement_tmp, 3)
            
            single_trial_eeg_tmp = squeeze(supplement_tmp(:, :, trialIdx));
        ...
        [w_tmp, v_tmp] = trca(transferred_eeg_tmp);
