...
% Evaluate the stcca performance on BETA dataset
...
close all;
% addpath('../mytoolbox');
% str_dir='/data/2020_BETA_SSVEP_database/';
% addpath('..\mytoolbox\');
str_dir='..\BETA SSVEP dataset\';

Fs=250;
% ch_used=[1:64]; % Pz, PO5, PO3, POz, PO4, PO6, O1, Oz, O2
ch_used=[48 54 55 56 57 58 61 62 63]; % Pz, PO5, PO3, POz, PO4, PO6, O1, Oz, O2
num_of_trials=3;                    % number of calibration trials per stimulus from target subject   
...
t_length0=0.2;
delta_t=0.1;
t_length=1.0;                         % analysis window length ([0.2:0.1:1.0])
...
for sn=1:70
    ...
    TW=t_length0:delta_t:t_length;          % Data length   
    TW_p=round(TW*Fs);
    ...
    save_name=['beta_stcca_acc_0216a_' num2str(num_of_signal_templates) '_' ...
    ...
    save(save_name,'num_of_signal_templates','num_of_signal_templates2',...
        ...
        'dataset_no','save_acc','num_of_trials','filename');    
    disp(sn)
    clear data1
end
